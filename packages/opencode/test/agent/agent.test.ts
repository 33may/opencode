...
it.instance("augusttrainer agent has ML autoresearch guardrails", () =>
  Effect.gen(function* () {
    ...
    expect(trainer?.prompt).toContain("Verifier protocol")
    expect(trainer?.prompt).toContain("suspiciously perfect scores")
    expect(trainer?.prompt).toContain("Dashboard, plots, and long-run logging")
    expect(trainer?.prompt).toContain("Training profiles and promotion ladders")
    expect(trainer?.prompt).toContain("prediction-market or arbitrage RL")
    expect(trainer?.prompt).toContain("Never tune hyperparameters on test, paper, live-paper, or real-market metrics")
